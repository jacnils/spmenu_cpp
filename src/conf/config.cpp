...
int bind_init(void) {
    ...
    if (!bindsfile || !strcmp(bindsfile, "NULL")) {
        if (!(xdg_conf = getenv("XDG_CONFIG_HOME"))) {
            ...
            if (!(bindfile = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s", home, ".config/spmenu/binds.conf") + 1)))) {
                die("spmenu: failed to malloc bindfile");
            }

            sprintf(bindfile, "%s/%s", home, ".config/spmenu/binds.conf");
        } else {
            if (!(bindfile = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s", xdg_conf, "spmenu/binds.conf") + 1)))) {
                die("spmenu: failed to malloc bindfile");
            }
        ...
        if (!(bindfile = static_cast<char*>(malloc(snprintf(NULL, 0, "%s", bindsfile) + 1)))) {
            die("spmenu: failed to malloc bindfile");
        }
...
void conf_init(void) {
    ...
    if (!configfile) {
        if (!(xdg_conf = getenv("XDG_CONFIG_HOME"))) {
            ...
            // malloc
            if (!(cfgfile = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s", home, ".config/spmenu/spmenu.conf") + 1)))) {
            ...
            // malloc
            if (!(cfgfile = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s", xdg_conf, "spmenu/spmenu.conf") + 1)))) {
                die("spmenu: failed to malloc cfgfile");
            }
        ...
        if (!(cfgfile = static_cast<char*>(malloc(snprintf(NULL, 0, "%s", configfile) + 1)))) {
            die("spmenu: failed to malloc cfgfile");
        }
...
void theme_load(void) {
    ...
    if (!themefile || !strcmp(themefile, "NULL")) {
        if (!(xdg_conf = getenv("XDG_CONFIG_HOME"))) {
            ...
            // malloc
            if (!(theme = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s", home, ".config/spmenu/theme.conf") + 1)))) {
            ...
            // malloc
            if (!(theme = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s", xdg_conf, "spmenu/theme.conf") + 1)))) {
                die("spmenu: failed to malloc theme");
            }
        ...
        if (!(theme = static_cast<char*>(malloc(snprintf(NULL, 0, "%s", themefile) + 1)))) {
            die("spmenu: failed to malloc theme");
        }

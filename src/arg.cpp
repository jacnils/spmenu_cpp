...
void screenshot(Arg *arg) {
    ...
    if (!screenshotfile) {
        ...
        if (!screenshotdir && !screenshotname) { // default
            if (!(file = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s-%02d-%02d-%02d%s", home, "spmenu-screenshot", t.tm_hour, t.tm_min, t.tm_sec, ".png") + 1)))) {
            ...
            if (!(file = static_cast<char*>(malloc(snprintf(NULL, 0, "%s/%s-%02d-%02d-%02d%s", screenshotdir, "spmenu-screenshot", t.tm_hour, t.tm_min, t.tm_sec, ".png") + 1)))) {
                die("spmenu: failed to malloc screenshot file");
            }
